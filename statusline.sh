...
if command -v jq >/dev/null 2>&1; then
  full_dir=$(echo "$input" | jq -r '.workspace.current_dir // .cwd // "unknown"' 2>/dev/null | sed "s|^$HOME|~|g")
  # Drop everything before Documents/GitHub - keep only from the main project directory onward
  current_dir=$(echo "$full_dir" | sed 's|.*/Documents/GitHub/||' | sed 's|.*/documents/github/||i')
  ...
  if [ -n "$blocks_output" ]; then
    ...
    if [ -n "$active_block" ]; then
      ...
      if [ -n "$reset_time_str" ] && [ -n "$start_time_str" ]; then
        ...
        session_txt="$(printf '%dh %dm until %s (%d%%)' "$rh" "$rm" "$end_hm" "$session_pct")"
